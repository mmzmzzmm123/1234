...
@RocketMQMessageListener(topic = MqConstants.TOPIC_ORDER_START_NOTICE, consumerGroup = MqConstants.GROUP_ORDER_START_NOTICE, selectorExpression = "*")
public class OrderStartNoticeMqConsumer implements RocketMQListener<Long> {
    ...
    public void onMessage(Long orderId) {
        log.info("mq消费-服务开始通知：开始，参数：{}", orderId);
        OrderInfo orderInfo = orderInfoMapper.selectOrderInfoById(orderId);
        if (ObjectUtil.isNull(orderInfo)){
            log.warn("mq消费-服务开始通知：失败，查询订单信息为空");
            throw new ServiceException("mq消费-服务开始通知：失败，查询订单信息为空");
        }
        // 客户数据通知
        ...
        if (StringUtils.isNotBlank(customUserUnionId)) {
            ...
            if (StringUtils.isNotBlank(customUserOpenId)) {
                ...
                dataMap.put("character_string6", orderInfo.getOrderNo());
                dataMap.put("time4", DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM, orderInfo.getOrderServiceTime()));
                ...
                userMessage.setTemplateId(WxMpTemplateIdConstants.ORDER_START);
