package com.jjpt.business.annotation;
...
import com.jjpt.business.utils.Reflections;
...
public class DictAspect {
     ...
     * @throws Throwable
     */
    @Around("execution(public *  com.jjpt.business..*.*Controller.*(..))")
    ...
    private void parseFullDictText(Object result) {
        ...
        try {
            ...
            Object rest = ((AjaxResult) result).get("data");
            ...
            if (rest instanceof List) {
                ...
                // 重新回写值
                ((AjaxResult) result).put("data",items);
                return;
            }

            // 处理单对象
            Object item = this.parseObject(((AjaxResult) result).get("data"));
            ((AjaxResult) result).put("data",item);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
