    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['course:label:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listLabel, getLabel, delLabel, addLabel, updateLabel } from "@/api/course/label";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除label编号为"' + ids + '"的数据项？').then(function() {
        return delLabel(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('course/label/export', {
        ...this.queryParams
