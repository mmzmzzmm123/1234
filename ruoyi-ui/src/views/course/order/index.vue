    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['course:order:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
...
<script>
import { listOrder, getOrder, delOrder, addOrder, updateOrder } from "@/api/course/order";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除课程订单编号为"' + ids + '"的数据项？').then(function() {
        return delOrder(ids);
      }).then(() => {
        this.getList();
