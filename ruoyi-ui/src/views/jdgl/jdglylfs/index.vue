    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jdgl:jdglylfs:remove']"
        >删除</el-button>
      </el-col>
	  <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="jdglylfsList" @selection-change="handleSelectionChange">
...
<script>
import { listJdglylfs, getJdglylfs, delJdglylfs, addJdglylfs, updateJdglylfs, exportJdglylfs } from "@/api/jdgl/jdglylfs";
...
export default {
  ...
  created() {
    this.getList();
    this.getDicts("sys_dm_jdglylfsrwmc").then(response => {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listJdglylfs(this.queryParams).then(response => {
        this.jdglylfsList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加基地管理引领辐射";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getJdglylfs(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateJdglylfs(this.form).then(response => {
            ...
            addJdglylfs(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除基地管理引领辐射编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delJdglylfs(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有基地管理引领辐射数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportJdglylfs(queryParams);
        }).then(response => {
