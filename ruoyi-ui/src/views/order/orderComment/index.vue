  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="是否展示" prop="ifShow">
        <el-select v-model="queryParams.ifShow" placeholder="请选择是否展示" clearable>
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['order:orderComment:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['order:orderComment:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['order:orderComment:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['order:orderComment:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="orderCommentList" @selection-change="handleSelectionChange">
...
<script>
import { listOrderComment, getOrderComment, delOrderComment, addOrderComment, updateOrderComment } from "@/api/order/orderComment";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOrderComment(this.queryParams).then(response => {
        this.orderCommentList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getOrderComment(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateOrderComment(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOrderComment(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除订单评论管理编号为"' + ids + '"的数据项？').then(function() {
        return delOrderComment(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('order/orderComment/export', {
        ...this.queryParams
      }, `orderComment_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
