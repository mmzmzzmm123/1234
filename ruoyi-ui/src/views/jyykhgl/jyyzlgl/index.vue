    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['jyykhgl:jyyzlgl:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['jyykhgl:jyyzlgl:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jyykhgl:jyyzlgl:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jyykhgl:jyyzlgl:export']"
        >导出</el-button>
      </el-col>
	  <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="jyyzlglList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="学科" align="center" prop="xk" />
...
<script>
import { listJyyzlgl, getJyyzlgl, delJyyzlgl, addJyyzlgl, updateJyyzlgl, exportJyyzlgl } from "@/api/jyykhgl/jyyzlgl";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listJyyzlgl(this.queryParams).then(response => {
        this.jyyzlglList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加质量管理（教研员）";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getJyyzlgl(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改质量管理（教研员）";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateJyyzlgl(this.form).then(response => {
            ...
            addJyyzlgl(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除质量管理（教研员）编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delJyyzlgl(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有质量管理（教研员）数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportJyyzlgl(queryParams);
        }).then(response => {
