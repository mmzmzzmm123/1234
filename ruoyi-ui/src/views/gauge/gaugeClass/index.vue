    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['psychology:gaugeClass:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['psychology:gaugeClass:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['psychology:gaugeClass:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['psychology:gaugeClass:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listGaugeClass, getGaugeClass, delGaugeClass, addGaugeClass, updateGaugeClass } from "@/api/gauge/gaugeClass";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getGaugeClass(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateGaugeClass(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGaugeClass(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('psychology/gaugeClass/export', {
        ...this.queryParams
      }, `gaugeClass_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
