    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['staff:staffGiftRecord:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['staff:staffGiftRecord:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['staff:staffGiftRecord:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['staff:staffGiftRecord:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="staffGiftRecordList" @selection-change="handleSelectionChange">
...
<script>
import { listStaffGiftRecord, getStaffGiftRecord, delStaffGiftRecord, addStaffGiftRecord, updateStaffGiftRecord } from "@/api/staff/staffGiftRecord";

export default {
  name: "StaffGiftRecord",
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      ...
      listStaffGiftRecord(this.queryParams).then(response => {
        this.staffGiftRecordList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateStaffGiftRecord(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStaffGiftRecord(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('staff/staffGiftRecord/export', {
        ...this.queryParams
      }, `staffGiftRecord_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
