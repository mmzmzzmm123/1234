<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['psychology:column:add']"
        >新增</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="columnList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="简介" prop="info" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['psychology:column:edit']"
          >编辑</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['psychology:column:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="封面" prop="img">
          <my-cropper v-model="form.img" sizeTip="宽132px 高130px" :extraData="extraData" :width="132" :height="130"/>
        </el-form-item>
        <el-form-item label="标题" prop="title">
          <el-input v-model="form.title" placeholder="请输入标题" maxlength="100" show-word-limit/>
        </el-form-item>
        <el-form-item label="简介" prop="info">
          <el-input type="textarea" :rows="3" size="mini" maxlength="255" show-word-limit v-model="form.info" placeholder="请输入简介" />
        </el-form-item>
...
import { getList, getOne, del, add, update } from "@/api/psychology/column";
...
export default {
  ...
  data() {
    return {
      ...
      extraData: {
        ...
        type: this.$constants['picTypes'][6]
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  methods: {
    ...
    handleStatus(row) {
      const that = this
      const content = row.status === '0' ? '确认要显示该咨询专栏吗？' : '确认要隐藏该咨询专栏吗？'
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$modal.confirm('是否确认删除咨询专栏为"' + row.title + '"的数据项？').then(function() {
        return del(row.id);
      }).then(() => {
        this.getList();
