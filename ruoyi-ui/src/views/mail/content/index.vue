...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        type: [
          { required: true, message: "类型1：晚安语     类型0:不能为空", trigger: "change" }
        ],
        isDelete: [
          { required: true, message: "0 无问题  1 删除不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除晚安语列表编号为"' + ids + '"的数据项？').then(function() {
