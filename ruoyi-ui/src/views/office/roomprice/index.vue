    ...
    <el-row :gutter="10" class="mb8">
     ...
     <!-- <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['office:roomprice:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['office:roomprice:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listRoomprice, getRoomprice, delRoomprice, addRoomprice, updateRoomprice } from "@/api/office/roomprice";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRoomprice(this.queryParams).then(response => {
        this.roompriceList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateRoomprice(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRoomprice(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除房间价格编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('office/roomprice/export', {
        ...this.queryParams
