    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:ztkzbhzrjgb:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:ztkzbhzrjgb:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:ztkzbhzrjgb:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['system:ztkzbhzrjgb:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="ztkzbhzrjgbList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="经营年限" align="center" prop="jynx" />
      <el-table-column label="法人年龄" align="center" prop="frnl" />
      <el-table-column label="注册地址" align="center" prop="zcdz" />
      <el-table-column label="重大税收违法案件" align="center" prop="zdsffwaj" />
      <el-table-column label="失信被执行人" align="center" prop="sxbzxr" />
      <el-table-column label="失信黑名单" align="center" prop="sfhmd" />
      <el-table-column label="国家下发工商异常名录" align="center" prop="gjxfgsycml" />
...
<script>
import { listZtkzbhzrjgb, getZtkzbhzrjgb, delZtkzbhzrjgb, addZtkzbhzrjgb, updateZtkzbhzrjgb, exportZtkzbhzrjgb } from "@/api/system/ztkzbhzrjgb";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        lyyyxtid: [
          { required: true, message: "来源应用系统编号不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listZtkzbhzrjgb(this.queryParams).then(response => {
        this.ztkzbhzrjgbList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加个体工商户信贷_主题库_准入监管";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const zjlid = row.zjlid || this.ids
      getZtkzbhzrjgb(zjlid).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改个体工商户信贷_主题库_准入监管";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.zjlid != null) {
            updateZtkzbhzrjgb(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addZtkzbhzrjgb(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除个体工商户信贷_主题库_准入监管编号为"' + zjlids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delZtkzbhzrjgb(zjlids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有个体工商户信贷_主题库_准入监管数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportZtkzbhzrjgb(queryParams);
        }).then(response => {
