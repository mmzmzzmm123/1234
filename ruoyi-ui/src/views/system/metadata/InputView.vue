<template>
  <div id="InputView" v-loading="loading">
    <div v-for="formConf in formConfs">
...
export default {
  ...
  methods:{
    loadForm(){
      this.loading = true
      // let query = {id:'',parentName: this.parentName,node:this.node}
      // listJson(query).then(res => {
      //   let array = res.rows;
      //   for (let i=0;i<array.length;i++){
      //     this.formConfs[i]=JSON.parse(array[i].formData)
      //   }
      //   console.log(this.formConfs)
      // })
      // // 回填数据
      // //this.fillFormData(this.formConfs, data)
      // this.key2 = +new Date()
      this.formConfs[0]={
        fields: [
          {
