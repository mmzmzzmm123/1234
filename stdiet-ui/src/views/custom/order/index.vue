...
export default {
  ...
  data() {
    ...
    const checkOrderTime = (rule, value, callback) => {
      ...
      if (
        dayjs(this.form.orderTime).diff(dayjs(this.form.becomeFanTime), "day") <
        0
      ) {
  ...
  methods: {
    ...
    reset() {
      ...
      const defaultOperator = this.operatorIdOptions.find(
        (opt) => opt.remark === "default"
      );
      const defaultOperatorAssis = this.operatorAssisIdOptions.find(
        (opt) => opt.remark === "default"
      );
      ...
      const defaultPlanner = this.plannerIdOptions.find(
        (opt) => opt.remark === "default"
      );
      const defaultPlannerAssis = this.plannerAssisIdOptions.find(
        (opt) => opt.remark === "default"
      );
      this.form = {
        orderId: null,
        ...
        plannerAssisId: defaultPlannerAssis
          ? parseInt(defaultPlannerAssis.dictValue)
          : null,
        operatorId: defaultOperator
          ? parseInt(defaultOperator.dictValue)
          : null,
        operatorAssisId: defaultOperatorAssis
          ? parseInt(defaultOperatorAssis.dictValue)
          : null,
        recommender: null,
