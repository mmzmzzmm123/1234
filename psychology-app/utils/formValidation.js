...
const form = {
	...
	*/
			...
			for (let i = 0, length = rule.length; i < length; i++) {
				...
				switch (ruleItem) {
					...
					case "isIdCard":
						...
						break;
					case "_isPassport":
						isError = !form._isPassport(formData[key]);
						break;
					case "_isPermits":
						isError = !form._isPermits(formData[key]);
						break;
					case "isAmount":
	...
	_isPassport: function (value) {
		const reg= /(^[EeKkGgDdSsPpHh]\d{8}$)|(^(([Ee][a-fA-F])|([DdSsPp][Ee])|([Kk][Jj])|([Mm][Aa])|(1[45]))\d{7}$)/;  //护照（包括香港和澳门）
		return reg.test(value);
	},
	_isPermits: function (value) {
		const reg= /^[a-zA-Z]\d{8}$|^\d{15}$|^\d{17}(\d|x|X)$/;  //回乡证
		return reg.test(value);
	},
