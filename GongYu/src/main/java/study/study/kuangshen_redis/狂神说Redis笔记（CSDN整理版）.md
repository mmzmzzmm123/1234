...
| `GETSET key value`                   | 将给定 key 的值设为 value ，并返回 key 的旧值(old value)。   | 127.0.0.1:6379> GETSET msg lixinger.mybatis.test “hello world”                |
| `SETNX key value`                    | 仅当key不存在时进行set                                       | 127.0.0.1:6379> SETNX msg lixinger.mybatis.test (integer) 0 127.0.0.1:6379> SETNX name sakura (integer) 1 |
