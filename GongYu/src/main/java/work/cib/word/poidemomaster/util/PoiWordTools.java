package work.cib.word.poidemomaster.util;
...
public class PoiWordTools {
    ...
    public static void replaceRadarCharts(POIXMLDocumentPart poixmlDocumentPart,
                                          List<String> titleArr, List<String> fldNameArr, List<Map<String, String>> listItemsByType) throws IOException {
    ...
    public static void replaceBarCharts(POIXMLDocumentPart poixmlDocumentPart,
        ...
        // 设置标题
        new work.cib.word.poidemomaster.util.PoiWordTitle().setBarTitle(ctChart, "我是修改后的标题");
    ...
    public static boolean refreshExcel(XWPFChart chart,
                                       List<Map<String, String>> dataList, List<String> fldNameArr, List<String> titleArr) throws IOException {
        boolean result = true;
    ...
    public static void setWordCellSelfStyle(XWPFTableCell cell, String fontName, String fontSize, int fontBlod,
        ...
        List<CTFonts> pfont = pRpr.getRFontsList();//获取<w:rPr>里的<w:rFonts w:ascii="宋体" w:eastAsia="宋体" w:hAnsi="宋体"/>
        if (pfont.size() == 0) {//没有<w:rPr>，创建
            pfont.add(pRpr.addNewRFonts());
        }
        //设置字体
        pfont.get(0).setAscii(fontName);
        pfont.get(0).setEastAsia(fontName);
        pfont.get(0).setHAnsi(fontName);

        List<CTOnOff> pb = pRpr.getBList();//获取<w:rPr>里的<w:b/>
        if (pb.size() == 0) {//没有<w:b/>，创建
            pb.add(pRpr.addNewB());
        }
        //设置字体是否加粗 龚宇把下面代码进行了注释
//        pb.setVal(fontBlod == 1 ? STOnOff.ON : STOnOff.OFF);
        List<CTHpsMeasure> psz = pRpr.getSzList();//获取<w:rPr>里的<w:sz w:val="32"/>
        if (psz.size() == 0) {//没有<w:sz w:val="32"/>，创建
            psz.add(pRpr.addNewSz());
        }
        // 设置单元格字体大小
        psz.get(0).setVal(bFontSize);

        List<CTHpsMeasure> pszCs = pRpr.getSzCsList();//获取<w:rPr>里的<w:szCs w:val="32"/>
        if (pszCs.size() == 0) {//没有<w:szCs w:val="32"/>，创建
            pszCs.add(pRpr.addNewSzCs());
        }
        // 设置单元格字体大小
        pszCs.get(0).setVal(bFontSize);
        //  <<--pRpr结束--
        //<<<---ppr结束---
        ...
        //获取<w:rPr>里的<w:rFonts w:ascii="宋体" w:eastAsia="宋体" w:hAnsi="宋体" w:hint="eastAsia"/>
        List<CTFonts> font = rpr.getRFontsList();
        if (font.size() == 0) {//没有<w:rFonts>，创建
            font.add(rpr.addNewRFonts());
        }
        //设置字体
        font.get(0).setAscii(fontName);
        font.get(0).setEastAsia(fontName);
        font.get(0).setHAnsi(fontName);

        List<CTOnOff> b = rpr.getBList();//获取<w:rPr>里的<w:b/>
        if (b.size() == 0) {//没有<w:b/>，创建
            b.add(rpr.addNewB());
        }
        //设置字体是否加粗  龚宇把下面代码进行了注释
        //b.setVal(fontBlod == 1 ? STOnOff.ON : STOnOff.OFF);
        List<CTColor> color = rpr.getColorList();  //获取<w:rPr>里的<w:color w:val="FFFFFF" w:themeColor="background1"/>
        if (color.size() == 0) {  //没有<w:color>，创建
            color.add(rpr.addNewColor());
        }
        // 设置字体颜色
        if (content.contains("↓")) {
            color.get(0).setVal("43CD80");
        } else if (content.contains("↑")) {
            color.get(0).setVal("943634");
        } else {
            color.get(0).setVal(fontColor.substring(1));
        }
        List<CTHpsMeasure> sz = rpr.getSzList();
        if (sz.size() == 0) {
            sz.add(rpr.addNewSz());
        }
        sz.get(0).setVal(bFontSize);
//        CTHpsMeasure szCs = rpr.getSzCs();
        List<CTHpsMeasure> szCs = rpr.getSzCsList();
        if (szCs.size() == 0) {
            szCs.add(rpr.addNewSz());
        }
        szCs.get(0).setVal(bFontSize);
        //-<-
        //<<--rpr结束--
