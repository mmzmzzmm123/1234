...
public class WordUtils_5 {
    public static void main(String[] args) throws Exception {
        WordUtils_5 t2 = new WordUtils_5();
        t2.createTable();
    }
    ...
    public void setParagraphRunFontInfo(XWPFParagraph p, XWPFRun pRun,
        ...
        // 设置字体
        List<CTFonts> fonts = pRpr.getRFontsList();//获取<w:rPr>里的<w:rFonts w:ascii="宋体" w:eastAsia="宋体" w:hAnsi="宋体"/>
        if (fonts.size() == 0) {//没有<w:rPr>，创建
            fonts.add(pRpr.addNewRFonts());
        }
        if (StringUtils.isNotBlank(enFontFamily)) {
            fonts.get(0).setAscii(enFontFamily);
            fonts.get(0).setHAnsi(enFontFamily);
        }
        if (StringUtils.isNotBlank(cnFontFamily)) {
            fonts.get(0).setEastAsia(cnFontFamily);
            fonts.get(0).setHint(STHint.EAST_ASIA);
        }
        // 设置字体大小
        List<CTHpsMeasure> sz = pRpr.getSzList();//获取<w:rPr>里的<w:sz w:val="32"/>
        if (sz.size() == 0) {//没有<w:sz w:val="32"/>，创建
            sz.add(pRpr.addNewSz());
        }
        sz.get(0).setVal(new BigInteger(fontSize));

        List<CTHpsMeasure> szCs = pRpr.getSzCsList();//获取<w:rPr>里的<w:szCs w:val="32"/>
        if (szCs.size() == 0) {//没有<w:szCs w:val="32"/>，创建
            szCs.add(pRpr.addNewSzCs());
        }
        szCs.get(0).setVal(new BigInteger(fontSize));
        ...
        if (isShd) {
            // 设置底纹
            List<CTShd> shd = pRpr.getShdList();
            if (shd.size() == 0) {
                shd.add(pRpr.addNewShd());
            }

//            CTShd shd = pRpr.isSetShd() ? pRpr.getShd() : pRpr.addNewShd();
            if (shdStyle != null) {
                shd.get(0).setVal(shdStyle);
            }
            if (shdColor != null) {
                shd.get(0).setColor(shdColor);
                shd.get(0).setFill(shdColor);
            }
        }
        ...
        if (spacingValue > 0) {
            // 设置字符间距信息
            List<CTSignedTwipsMeasure> ctSTwipsMeasure = pRpr.getSpacingList();
            if (ctSTwipsMeasure.size() == 0) {
                ctSTwipsMeasure.add(pRpr.addNewSpacing());
            }
            ctSTwipsMeasure.get(0).setVal(new BigInteger(String.valueOf(spacingValue)));
        }
        if (indent > 0) {
            List<CTTextScale> paramCTTextScale = pRpr.getWList();
            if (paramCTTextScale.size() == 0) {
                paramCTTextScale.add(pRpr.addNewW());
            }

            paramCTTextScale.get(0).setVal(indent);
        }
    }
...
//            CTSignedTwipsMeasure ctSTwipsMeasure = pRpr.isSetSpacing() ? pRpr.getSpacing() : pRpr.addNewSpacing();
    ...
    public void createTable() throws Exception {
        ...
        setTableWidthAndHAlign(table, "9024", STJcTable.CENTER);
        ...
        saveDocument(xdoc, "F:\\leecode\\src\\main\\java\\cib\\word\\resources\\" + "WordUtils_6.docx");
    }
}
