...
public class ManagerClassGeneratePlugin extends AbstractClassPlugin {
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
		...
		if (!existClass(manager)) {
			list.add(manager);
			log.info("创建Manager Java文件={}/{}.{}", manager.getTargetProject(), manager.getTargetPackage(),
					manager.getFileName());
		}
		else if (isOverride(introspectedTable)) {
			list.add(manager);
			log.info("覆盖Manager Java文件={}/{}.{}", manager.getTargetProject(), manager.getTargetPackage(),
					manager.getFileName());
		}
		else {
