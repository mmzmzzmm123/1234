...
public class DaoClassGeneratePlugin extends AbstractClassPlugin {
	...
	public List<GeneratedJavaFile> contextGenerateAdditionalJavaFiles(IntrospectedTable introspectedTable) {
		...
		if (!existClass(daoInterface)) {
			...
			log.info("创建DAO Java文件={}/{}.{}", daoInterface.getTargetProject(), daoInterface.getTargetPackage(),
					daoInterface.getFileName());
		}
		else if (isOverride(introspectedTable)) {
			list.add(daoInterface);
			log.info("覆盖DAO Java文件={}/{}.{}", daoInterface.getTargetProject(), daoInterface.getTargetPackage(),
					daoInterface.getFileName());
		}
		else {
			log.info("跳过DAO Java文件={}/{}.{}", daoInterface.getTargetProject(), daoInterface.getTargetPackage(),
					daoInterface.getFileName());
		}
