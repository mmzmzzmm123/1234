...
public class GeneratorSqlPlugin extends AbstractClassPlugin {
    ...
    public void setContext(Context context) {
        ...
        file = new File(
            String.format("{}/{}.sql", context.getProperty(Backup.SQL_TARGET_PATH), date));
        // todo
        // Files.newFile(file);
            ...
            try (Connection connection = factory.getConnection()) {
                try (Statement statement = connection.createStatement()) {
                    ResultSet result = statement
                        .executeQuery("show create table "
                    ...
                    if (result.next()) {
                        ...
                        if (indexStart > 0) {
                            ...
                            tableSql = tableSql.substring(0, indexStart)
                                       + tableSql.substring(indexEnd);
                        }
                        // 把自增主键换回来
                        // todo
