...
public class MybatisGenServiceImpl {
	...
	public void generatorCode(String tableName) {
		...
		templates.removeAll(Arrays
				.asList(new String[] { "vm/java/domain.java.vm", "vm/java/mapper.java.vm", "vm/xml/mapper.xml.vm" }));
			...
			if (!StringUtils.containsAny(template, "sql.vm", "api.js.vm", "index.vue.vm", "index-tree.vue.vm")) {
				...
				try {
					...
					if (template.endsWith(".java.vm")) {
						FileEdit fileEdit = fileFormatter.formatFile(file, Charset.forName(CharsetKit.UTF_8));
						fileEdit.save();
					}
				}
				catch (IOException e) {
