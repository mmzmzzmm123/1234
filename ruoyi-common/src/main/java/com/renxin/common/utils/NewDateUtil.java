...
public final class NewDateUtil {
    ...
    public static String nowDate() {
        return nowDate(NewConstants.DATE_FORMAT_NUM);
    }
    ...
    public static Date nowDateTime() {
        return strToDate(nowDateTimeStr(), NewConstants.DATE_FORMAT);
    }
    ...
    public static String nowDateTimeStr() {
        return nowDate(NewConstants.DATE_FORMAT);
    }
    ...
    public static long getTwoDateDays(Date beforeDay, Date afterDay) {
        SimpleDateFormat sm = new SimpleDateFormat(NewConstants.DATE_FORMAT_NUM);
        long days = -1;
        try {
    ...
    public static int getSecondTimestamp(String date){
        ...
        Date date1 = strToDate(date, NewConstants.DATE_FORMAT);
        if(date1 == null){
            return 0;
        }
    ...
    public static int getSecondTimestamp(){
        Date date = strToDate(nowDateTime(NewConstants.DATE_FORMAT), NewConstants.DATE_FORMAT);
        if (null == date) {
            return 0;
        }
        String timestamp = String.valueOf(date.getTime()/1000);
    ...
    public static String getWeekEndDay() {
        return addDay(getWeekStartDay(), 7, NewConstants.DATE_FORMAT);
    }
    ...
    public static String getLastWeekEndDay() {
        return addDay(getLastWeekStartDay(), 7, NewConstants.DATE_FORMAT);
    }
    ...
    public static DateLimitUtilVO getDateLimit(String data){
        ...
        String day = NewDateUtil.nowDateTime(NewConstants.DATE_FORMAT_START);
        String end = NewDateUtil.nowDateTime(NewConstants.DATE_FORMAT_END);

        if(!StringUtils.isBlank(data)){
            switch (data){
                case NewConstants.SEARCH_DATE_DAY:
                    startTime = day;
                    break;
                case NewConstants.SEARCH_DATE_YESTERDAY:
                    startTime = NewDateUtil.addDay(day, -1, NewConstants.DATE_FORMAT_START);
                    endTime = NewDateUtil.addDay(end, -1, NewConstants.DATE_FORMAT_END);
                    break;
                case NewConstants.SEARCH_DATE_LATELY_7:
                    startTime = NewDateUtil.addDay(day, -6, NewConstants.DATE_FORMAT_START);
                    break;
                case NewConstants.SEARCH_DATE_WEEK:
                    startTime = getWeekStartDay();
                    endTime = getWeekEndDay();
                    break;
                case NewConstants.SEARCH_DATE_PRE_WEEK:
                    startTime = getLastWeekStartDay();
                    endTime = getLastWeekEndDay();
                    break;
                case NewConstants.SEARCH_DATE_LATELY_30:
                    startTime = NewDateUtil.addDay(day, -30, NewConstants.DATE_FORMAT_START);
                    break;
                case NewConstants.SEARCH_DATE_MONTH:
                    startTime = NewDateUtil.nowDateTime(NewConstants.DATE_FORMAT_MONTH_START);
                    endTime = getMonthEndDay();
                    break;
                case NewConstants.SEARCH_DATE_PRE_MONTH:
                    startTime = getLastMonthStartDay();
                    endTime = getLastMonthEndDay();
                    break;
                case NewConstants.SEARCH_DATE_YEAR:
                    startTime = NewDateUtil.nowDateTime(NewConstants.DATE_FORMAT_YEAR_START);
                    endTime = NewDateUtil.nowDateTime(NewConstants.DATE_FORMAT_YEAR_END);
                    break;
                case NewConstants.SEARCH_DATE_PRE_YEAR:
                    startTime = getLastYearStartDay();
                    endTime = getLastYearEndDay();
                    break;
                default:
                    List<String> list = ComUtil.stringToArrayStr(data);
                    if(list.size() == 1){
                        throw new UtilException("选择时间参数格式错误，请在 " +
                                NewConstants.SEARCH_DATE_DAY + "|" +
                                NewConstants.SEARCH_DATE_YESTERDAY + "|" +
                                NewConstants.SEARCH_DATE_LATELY_7 + "|" +
                                NewConstants.SEARCH_DATE_LATELY_30 + "|" +
                                NewConstants.SEARCH_DATE_MONTH + "|" +
                                NewConstants.SEARCH_DATE_YEAR + "|自定义时间范围（格式：yyyy-MM-dd HH:mm:ss，两个时间范围用逗号分割）");
                    }
                    startTime = list.get(0);
                    endTime = list.get(1);

//                    if (startTime.equals(endTime)) {
                        startTime = NewDateUtil.appointedDayStrToFormatStr(startTime, NewConstants.DATE_FORMAT_DATE, NewConstants.DATE_FORMAT_START);
                        endTime = NewDateUtil.appointedDayStrToFormatStr(endTime, NewConstants.DATE_FORMAT_DATE, NewConstants.DATE_FORMAT_END);
//                    }
                    break;
            }
        }
    ...
    public static List<String> getListDate(String data) {
        ...
        //开始日期
        Date startDate = NewDateUtil.strToDate(dateLimit.getStartTime(), NewConstants.DATE_FORMAT);

        //结束日期
        Date endDate = NewDateUtil.strToDate(dateLimit.getEndTime(), NewConstants.DATE_FORMAT);

        while (endDate.after(startDate)){
            date.add(NewDateUtil.dateToStr(startDate, NewConstants.DATE_FORMAT_DATE)); // 放入集合
            startDate = NewDateUtil.strToDate(NewDateUtil.addDay(startDate, 1, NewConstants.DATE_FORMAT), NewConstants.DATE_FORMAT); //循环一次 加一天
        }
    ...
    public static String appointedDayStrToFormatStr(String dateStr, String STR_DATE_FORMAT, String DATE_FORMAT) {
        Date date = NewDateUtil.strToDate(dateStr, STR_DATE_FORMAT);
        return NewDateUtil.dateToStr(date, DATE_FORMAT);
    }
    ...
    public static String timestamp2DateStr(Long timestamp, String dateFormat) {
        ...
        if (StrUtil.isBlank(dateFormat)) {
            dateFormat = NewConstants.DATE_FORMAT;
        }
    ...
    public static Long dateStr2Timestamp(String dateStr, String type) {
        ...
        if (StrUtil.isNotBlank(type)) {
            if (type.equals(NewConstants.DATE_TIME_TYPE_BEGIN)) {
                parse = cn.hutool.core.date.DateUtil.beginOfDay(parse);
            }
            if (type.equals(NewConstants.DATE_TIME_TYPE_END)) {
