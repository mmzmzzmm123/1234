...
public class RedissonDelayQueue implements DelayQueue, Runnable {
	...
	public <T> void put(Task task) {
		...
		log.info("RedissonDelayQueue_put {}", JSON.toJSONString(task));
	}
...
//				RDelayedQueue<Task> rk = properties.getRedissonClient().getDelayedQueue(blockingQueue);
//				final Task task = properties.getRedissonClient().getDelayedQueue(rk).poll();
				log.info("RedissonDelayQueue_tack {}", JSON.toJSONString(task));
				...
				e.printStackTrace();
				log.error("RedissonDelayQueue_tack_error" , e);
			}
		}
	}
