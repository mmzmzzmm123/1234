...
public class TRoomServiceImpl extends ServiceImpl<TRoomMapper, TRoom> implements ITRoomService {
    ...
    public String openRoom(Long id) {
        ...
        TStore tStore = tStoreMapper.selectTStoreById(room.getStoreId());
        String hornName = "十三将";
        if (tStore != null && tStore.getName() != null) {
            hornName = tStore.getName();
        }
        ...
        for (String equip : equips.split(",")) {
            ...
            if (OfficeEnum.EquipType.HORN.getCode().equalsIgnoreCase(currentEq.getEquipType())) {
                ...
                CloudHornRegResponse resp = HornConfig.hornSend(currentEq.getEquipControl(), "已开门，欢迎使用" + hornName + "无人自助棋牌");
