...
import com.ruoyi.common.core.domain.result.ResultUserRole;
...
public class SysUserController extends BaseController {
    ...
    public Result getInfo(@PathVariable(value = "userId", required = false) Long userId) {
        ...
        ResultUserRole resultUserRole = ResultUserRole.rolesPosts(roles, this.postService.selectPostAll());
        if (StringUtils.isNotNull(userId)) {
            SysUser sysUser = this.userService.selectUserById(userId);
            resultUserRole.setIds(sysUser,
                    sysUser.getRoles().stream().map(SysRole::getRoleId).collect(Collectors.toList()),
                    this.postService.selectPostListByUserId(userId));
        }
    ...
    public Result add(@Validated @RequestBody SysUser user) {
        if (UserConstants.NOT_UNIQUE.equals(this.userService.checkUserNameUnique(user.getUserName()))) {
    ...
    public Result authRole(@PathVariable("userId") Long userId) {
        ...
        List<SysRole> rolesList = this.roleService.selectRolesByUserId(userId);
        List<SysRole> roles = SysUser.isAdmin(userId) ? rolesList : rolesList.stream().filter(r -> !r.isAdmin()).collect(Collectors.toList());
        return ResultUserRole.userRoles(user, roles);
    }
