...
public class CacheController {
...

    {
        ...
        caches.add(new SysCache(CacheConstants.USER_TOKENS_KEY, "用户登录列表"));
        ...
        try {
            ...
            if (dataType != null) {
                if (dataType == DataType.ZSET) {
                    Set<String> set = redisTemplate.opsForZSet().range(cacheKey, 0, -1);
                    cacheValue = set == null ? "" : set.stream().collect(Collectors.joining());
                }
            }
        }
